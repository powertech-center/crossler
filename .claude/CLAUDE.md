...
Пример полного конфига с описанием всех полей: `.claude/docs/example-config.toml`.
...
| Файл | Что внутри |
|------|------------|
| `specs/config.md` | **⬅ АКТИВНЫЙ РАБОЧИЙ ФАЙЛ** — проектирование формата конфига и CLI. Сейчас ведётся плотная работа над этим файлом. Именно здесь фиксируются все текущие решения и обсуждения |
| `specs/goreleaser.md` | Обзор GoReleaser — очень популярной утилиты для упаковки Go-проектов. Изучается как источник идей для Crossler: структура конфига, CLI, поддерживаемые форматы, подход к подписи и нотаризации |
| `specs/crossler-recommendations.md` | AI-рекомендации по конфигу и архитектуре Crossler — **не утверждены пользователем**, используются как отправная точка для рассуждений |
| `specs/signing-recommendations.md` | AI-рекомендации по реализации подписи в Crossler — **не утверждены пользователем**, используются как отправная точка для рассуждений |
| `specs/toml-example.toml` | Черновой пример конфига Crossler в TOML-формате |

**Рабочий процесс:** ближайший этап — детальная проработка формата конфигурационного файла и аргументов командной строки. Основной документ — `specs/config.md`.
