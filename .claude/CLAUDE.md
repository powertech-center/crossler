...
Формат файла конфигурации — **TOML**. Конфиг должен поддерживать **многоуровневое наслоение настроек**: общие параметры → переопределения для платформы → переопределения для архитектуры. Бинарники и сопроводительные файлы (иконки, документация) описываются раздельно, т.к. физически лежат в разных местах.
...
## Спецификации (.claude/specs/)

Директория для детальной проработки конкретных аспектов Crossler. Здесь формируются утверждённые спецификации — в отличие от `docs/`, где хранятся исследовательские материалы и предварительные рекомендации.

**Текущие файлы:**

| Файл | Что внутри |
|------|------------|
| `specs/synthesis.md` | **⬅ ГЛАВНЫЙ ДОКУМЕНТ** — утверждённые решения по конфигу и архитектуре Crossler. Философия: KISS + Бритва Оккама + YAGNI. Только факты от пользователя, никакой спекуляции |
| `specs/config.md` | Проектирование формата конфига и CLI — детальный анализ и открытые вопросы |
| `specs/goreleaser.md` | Обзор GoReleaser — источник идей для Crossler |
| `specs/crossler-recommendations.md` | AI-рекомендации по конфигу и архитектуре — **не утверждены пользователем** |
| `specs/signing-recommendations.md` | AI-рекомендации по реализации подписи — **не утверждены пользователем** |

**Рабочий процесс:** наполнение `specs/synthesis.md` — раздел «Интеграция».

**Текущий прогресс по разделу «Интеграция»:**

- ✅ **PATH** — зафиксировано: автоматическая доступность `bin` на всех платформах, без явного параметра
- ✅ **env** — зафиксировано: параметр-словарь для произвольных переменных окружения. Автоматически на Windows (.msi) и Linux-пакетах. Для остальных форматов — генерация скриптов в `/etc/profile.d/{slug}.*`
- ✅ **Деинсталляция macOS .pkg** — зафиксировано: нужен uninstall-скрипт
- 🔲 **Сервисы/демоны** — в обсуждении. Решено: поддержка нескольких сервисов на пакет, основной кейс — один бинарник с разными аргументами. Открытые вопросы: формат записи (секции `[...]` зарезервированы для таргетирования, нужна альтернатива), упрощённая форма для одного сервиса, полный набор полей, маппинг на systemd/launchd/Windows
- 🔲 Ярлыки / desktop-записи
- 🔲 Ассоциации файлов
- 🔲 Обработчики протоколов
- 🔲 Контекстное меню
- 🔲 Автозапуск
- 🔲 Shell completions
- 🔲 Скрипты (pre/post install/remove)
