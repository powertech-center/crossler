...
| `output` | string | `{current-dir}/{config-name}` | Директория для сохранения готовых пакетов | все бэкенды: выходная директория |
...
| `template` | string | `"{slug}-{version}-{os}-{arch}.{format}"` | Шаблон имени выходного файла | все бэкенды: имя выходного файла |

## Переменные шаблонизатора

Используются в строковых параметрах (`output`, `template` и др.) в виде `{name}`.
